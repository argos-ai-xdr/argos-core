...
| `independent-verifier` | ADR-055 (Fase H, actualizado K.1): Independent Verification Barrier entre `safety-kernel` y `policy-adapter`; re-confirma el `SafetyEnvelope` con hechos frescos, incluida la referencia de MissionContext sellada | Lógica real; `VERIFIED` alcanzable por un checkout real cuando se confirman todos los hechos frescos |
